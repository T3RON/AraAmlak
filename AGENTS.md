...
```
caller‌ها را قبل از تغییر بررسی کن.
...
### قانون ۴ — قبل از اعتماد به نتایج
```
...
فقط با تأیید مستقیم از منبع. گراف ایستا ارجاع‌های رشته‌ای (URL name، Celery task، signal، template tag، getattr) را نمی‌بیند.
```
...
- **تست‌ها:** قبل از هر push سبز باشد
